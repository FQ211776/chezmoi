...
Instalar_repositorio() {
    ...
    repo_dir="$HOME/Developments/github.com/folke"
...
Instalar_dotfiles() {
    ...
    repo_dir="$HOME/Developments/github.com/folke/dot"
    ...
    if [ -f ~/.config/starship.toml ]; then
        if mv ~/.config/starship.toml "$backup_folder"/starship.toml_"$date" 2>>RiceError.log; then
            printf "%s%sstarship.toml file backed up successfully at %s%s/starship.toml_%s%s\n" "${BLD}" "${CGR}" "${CBL}" "$backup_folder" "${date}" "${CNC}"
        else
            printf "%s%sFailed to backup starship.toml file. See %sRiceError.log%s\n" "${BLD}" "${CRE}" "${CBL}" "${CNC}"
        fi
    else
...
# for file in "$HOME"/.config/fstarship.toml; do
#         if [ -f "$file" ]; then
#             if mv "$file" "$backup_folder"/user.js_"$date" 2>>RiceError.log; then
#                 printf "%s%sstarship.toml file backed up successfully at %s%s/user.js_%s%s\n" "${BLD}" "${CGR}" "${CBL}" "$backup_folder" "${date}" "${CNC}"
#             else
#                 printf "%s%sFailed to backup starship.toml file. See %sRiceError.log%s\n" "${BLD}" "${CRE}" "${CBL}" "${CNC}"
#             fi
#         else
...
#     done
    ...
    # if cp -R $repo_dir/config/fish ~/.config/ 2>>RiceError.log; then
    #     printf "%s%sfish %sfolder copied succesfully!%s\n" "${BLD}" "${CYE}" "${CGR}" "${CNC}"
    #     sleep 15
    # else
    #     printf "%s%sfish %sfolder failed to been copied, see %sRiceError.log %sfor more details.%s\n" "${BLD}" "${CYE}" "${CRE}" "${CBL}" "${CRE}" "${CNC}"
    #     sleep 15
    # fi
    ...
    cp -fv "$repo_dir"/config/starship.toml "$HOME/.config/"
